...
*/
interface IHeroData{
    function summoner() external view responsible returns (Constants.Hero _hero,address approvedAddr);
}
...
IPausable, ITransferOwner, ISendSurplusGas, IVersioned {
    ...
    */
    constructor(uint256 root_public_key_, address root_owner_address_,string name_,string symbol_,TvmCell wallet_code_) public {
    ...
    */
    ...
    function  mintWithLvUp(uint256 level,address owner) public {
        require(msg.sender!=address (0x0));
        IHeroData(msg.sender).summoner{value: 0,flag:64,callback:RootTokenContract.onApprovedOrOwner}();
    }
    function onApprovedOrOwner(Constants.Hero hero,address approvedAddr)public{
        uint256  curent_level=hero.level-1;
        uint256 xp_level = curent_level * 1000;
        for (uint i = 1; i < curent_level; i++) {
            xp_level += i * 1000;
        }
        _deployWallet(uint128(xp_level*1e9),0.1 ton,0,hero.ownerAddr,hero.ownerAddr);
    }
    /*
    ...
    */
    ...
    _deployWallet(tokens,deploy_grams,wallet_public_key_,owner_address_,gas_back_address);
    }
